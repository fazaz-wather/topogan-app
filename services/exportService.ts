...
/**
 * Tente de sauvegarder un fichier en utilisant l'API File System Access (Enregistrer sous...).
...
export const saveFileWithPicker = async (
    ...
    try {
        ...
        if (err.name === 'AbortError') {
            return true; // Annulé par l'utilisateur, on considère traité pour éviter le fallback automatique
        }
        
        // Ignorer l'erreur spécifique aux iframes/cross-origin pour permettre le fallback silencieux
        if (err.name === 'SecurityError' || (err.message && err.message.includes('Cross origin sub frames'))) {
            console.warn("File System Access API non disponible (contexte iframe/sécurité). Passage au téléchargement classique.");
            return false;
        }

        console.error("Erreur File System Access:", err);
    }
    return false;
};
 ...
 * Sur mobile (supportant navigator.share avec des fichiers), ouvre la feuille de partage native.
 * Sur PC, tente showSaveFilePicker, sinon déclenche un téléchargement classique.
 */
export const saveAndShareFile = async (
    ...
    // Tentative de sauvegarde directe (Desktop moderne)
    const savedDirectly = await saveFileWithPicker(blob, filename, 'Fichier TOPOGAN', mimeType, extension);
    ...
    if (navigator.canShare && navigator.share) {
        try {
            ...
            console.warn("Le partage natif a été annulé ou a échoué, repli sur le téléchargement.", err);
        }
    }

    // Repli sur le téléchargement classique
...
export const exportPointsToCSV = (points: Point[], settings: AppSettings) => {
    ...
    const pointDisplayPrecision = coordinateSystem === 'wgs84' ? Math.max(6, precision) : precision;
    ...
    points.forEach(p => {
        csvRows.push(`${p.id},${p.x.toFixed(pointDisplayPrecision)},${p.y.toFixed(pointDisplayPrecision)}`);
    });

    const csvContent = csvRows.join('\r\n');
    saveAndShareFile(csvContent, 'export_topogan_points.csv', 'text/csv;charset=utf-8;');
};


export const exportSurfaceDataToCSV = (points: Point[], results: CalculationResults, settings: AppSettings) => {
  ...
  if (areaUnit === 'ha_a_ca') {
    ...
    csvRows.push(`Surface,${displayedArea.toFixed(precision)},${escapeCSV(areaLabel)}`);
  }
  ...
  results.distances.forEach(d => {
    ...
    csvRows.push(`${d.from},${d.to},${displayedDistance.toFixed(precision)},${escapeCSV(distanceLabel)}`);
  });
  
  const csvContent = csvRows.join('\r\n');
  saveAndShareFile(csvContent, 'export_topogan_surface.csv', 'text/csv;charset=utf-8;');
};
